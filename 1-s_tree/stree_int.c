...
void st_remove(struct st_node **root, struct st_node *del)
{
    if (st_right(del)) {
        ...
        st_replace_right(del, least);      // XXX: AAAA
        st_update(root, st_right(least));  // XXX: BBBB
        return;
    }
    ...
    if (st_left(del)) {
        ...
        st_replace_left(del, most);      // XXX: CCCC
        st_update(root, st_left(most));  // XXX: DDDD
        return;
    }
    ...
    st_update(root, parent);  // XXX: EEEE
}
...
#define streeint_entry(ptr) container_of(ptr, struct streeint, st_n)
...
static void __streeint_destroy(struct st_node *n)
{
    if (st_left(n))
        __streeint_destroy(st_left(n));

    if (st_right(n))
        __streeint_destroy(st_right(n));

    struct streeint *i = streeint_entry(n);
    free(i);
}
...
int streeint_destroy()
{
    assert(tree);
    if (st_root(tree))
        __streeint_destroy(st_root(tree));
...
struct streeint *streeint_insert(int a)
{
    ...
    for (struct st_node *n = st_root(tree); n;) {
        struct streeint *t = container_of(n, struct streeint, st_n);
        if (a == t->value)
            return t;
    ...
    struct streeint *i = calloc(sizeof(struct streeint), 1);
    if (st_root(tree))
...
struct streeint *streeint_find(int a)
{
    ...
    while (n) {
        struct streeint *t = streeint_entry(n);
        if (a == t->value)
            return t;
...
{
    struct streeint *n = streeint_find(a);
    if (!n)
        return -1;
...
/* ascending order */
static void __streeint_dump(struct st_node *n, int depth)
{
    if (!n)
        return;

    __streeint_dump(st_left(n), /* XXX: FFFF */ depth + 1);

    struct streeint *v = streeint_entry(n);
    printf("%d\n", v->value);

    __streeint_dump(st_right(n) /* XXX: GGGG */, depth + 1);
}

void streeint_dump()
{
    __streeint_dump(st_root(tree), 0);
}
